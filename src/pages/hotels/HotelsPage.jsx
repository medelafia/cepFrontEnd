...
import Hotel from "./componants/Hotel";
  ...
  const findNearby = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        fetch(
          `http://localhost:8089/hotels/getNearby?lng=${pos.coords.longitude}&lat=${pos.coords.latitude}`
        )
          .then((res) => res.json())
      ...
      {isLoading ? (
        ...
        <>
          <div className="w-100 py-3 d-flex align-items-center">
            <TextField
              label="hotel name or city"
              fullWidth
              defaultValue={search}
            />
            <button className="btn custom-btn-secondary ms-1 p-3">
              <i class="fa-solid fa-magnifying-glass"></i>
            </button>
            <span className="text-secondary mx-4">or</span>
            <button className="btn btn-outline-dark py-3" onClick={findNearby}>
              nearby
            </button>
          </div>
            ...
            <div className="col-md-12 my-3">
              ...
              <div className="my-4 row">
                {hotels?.length == 0 ? (
                  ...
                  <>
                    {renderHotels()}
                    {hotels?.length > nbElements && (
                      <ShowMore callBack={setNbElements(nbElements + 9)} />
                    )}
                  </>
                )}
              </div>
